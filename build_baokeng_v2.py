#!/usr/bin/env python3
"""ST保壳评分系统V2 批量构建管道 — 十二维100分制（老Z定稿版 2026-08-28，2026-09-02 13→12重构）
...
老Z 12维重构（2026-09-02，config v2.3）：
  取消F1经营改善趋势4分 → A1净资产+2分(10→12)、C2壳价值+2分(8→10)；F2重组更名F1；总分仍=100

| 维度 | 满分 | 数据源 |
|------|------|--------|
| C1 面值距离      | 6  | 腾讯实时价格 |
| C2 壳价值锚定    | 10 | 腾讯市值（壳基准SHELL_BASE=own市值中位数33.81亿） |
| S1 实控人性质    | 12 | 东财F10 ACTUAL_HOLDER（st_controllers.json）+ 前十大股东（st_top_holders.json） |
| S2 质押与控制权  | 6  | AkShare质押比例 + 巨潮冻结公告 |
| A1 净资产充裕度  | 12 | AkShare东方财富 |
| A2 扣非主营收入  | 12 | AkShare东方财富（扣非主营=F10主营构成口径） |
| A3 扣非盈利      | 6  | AkShare同花顺 |
| D1 现金流质量    | 4  | AkShare东方财富 |
| B1 立案/造假信号 | 10 | 巨潮公告 |
| B2 审计意见      | 12 | 巨潮公告 |
| F1 重组/纾困进度 | 6  | 巨潮公告（原F2更名） |
| H1 实控人司法    | 4  | 巨潮公告 |
| 合计             | 100 | |
...
payload = {
    'meta': {
        'system': 'ST保壳评分系统V2（老Z定稿2026-08-28，2026-09-02重构，十二维100分制）',
        'philosophy': '退市概率打分：维度=退市通道，权重=5年176家退市案例实证贡献度',
        'dims': 'C1面值6/C2壳价值10(规则反转,基准SHELL_BASE=33.81亿own口径)/S1实控人12(性质×持股×质押×诉讼连乘,涉造假封顶4)/'
                'S2质押6(中登周报)/A1净资产12/A2扣非主营收入12(F10主营构成口径)/A3扣非6/'
                'D1现金流4/B1立案造假10/B2审计12/F1重组纾困6(原F2,更名)/H1司法4',
        'levels': 'A(>70) B(51-70) C(31-50) D(≤30)，分数越高=保壳越容易；'
                  '联动规则：C1≤1时C2降档(面值危机压制壳价值)；涉造假立案S1封顶4',
        'config': '权重/档位/闸门/评级外置score_config_v2.json(v2.3，2026-09-02 13→12重构)；'
...
if mg:
    ...
    for k in ('C1', 'C2', 'S1', 'S2', 'A1', 'A2', 'A3', 'D1', 'B1', 'B2', 'F1', 'H1'):
        print(f'    {k}={mg[k]}', end='')
    print()
