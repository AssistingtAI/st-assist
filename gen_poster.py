# -*- coding: utf-8 -*-
"""生成公众号竖版长图HTML（750px宽）——保壳风云榜V2（十二维合规口径）"""
...
c2_full = [r for r in pool if r["C2"] == 10]
...
extra_labels = {"cap": "市值", "ctrl": "实控人", "moyu": "壳市值指数"}
...
# ---------- 等级分布条 ----------
lv_desc = {"A": "退市风险相对较低（>70）", "B": "中等风险（51-70）", "C": "风险较高（31-50）", "D": "退市风险极高（≤30）"}
...
# ---------- 五通道权重 ----------
chans = [("财务红线", 34, "净资产·扣非主营·扣非盈利·现金流"),
         ("监管类", 22, "立案造假·审计意见"),
         ("股东实力", 18, "实控人·质押"),
         ("市场类", 16, "面值·壳价值"),
         ("纾困与司法", 10, "重组纾困·司法")]
...
<div class="hero">
<div class="tag">V2 · 十二维 · 100分制</div>
<h1>保壳风云榜</h1>
...
<div class="st"><b>12</b><i>评分维度</i></div>
<div class="st"><b>{levels['A']}</b><i>A级·风险相对较低</i></div>
<div class="st"><b>176</b><i>退市案例回测样本</i></div>
</div>

<div class="card">
<div class="ct">等级分布<span class="cs">分数越高 = 退市风险相对较低</span></div>
{dist_rows}
</div>

{table_block("评分最高 TOP 10", "综合评分靠前 · 退市风险相对较低", easy_top, ctrl_extra, "实控人")}

{table_block("壳市值指数 TOP 10", "低市值 × 较高评分的组合观察（研究口径）", moyu_top, moyu_extra, "壳市值指数")}

{table_block("评分最低 TOP 10", "临近面值/财务红线/监管高压，退市风险较高区", hard_bottom, cap_extra, "市值")}

<div class="card">
<div class="ct">评分体系<span class="cs">五通道权重 = 5年176家退市案例实证贡献度</span></div>
{chans_html}
<div class="note" style="margin-top:12px">十二维：C1面值6 · C2壳价值10 · S1实控人12 · S2质押6 · A1净资产12 · A2扣非主营收入12 · A3扣非6 · D1现金流4 · B1立案造假10 · B2审计12 · F1重组纾困6 · H1司法4；联动闸门：面值危机压制壳价值、涉造假立案实控人维度封顶；通道封顶：C1=0或B2=0总分封顶50、B1=0封顶30。</div>
</div>

<div class="c2box">
<b>低市值观察（C2满分，{len(c2_full)}家）</b>
<p>市值 ≤ 16.9亿（壳基准33.8亿的五折）：{c2_names} 等。壳价值维度为反转计分口径——市值越低于基准，该维度得分越高。此为评分规则说明与数据观察，不构成任何投资建议或机会判断。</p>
</div>

<div class="foot">
<h3>完整榜单 · 个股12维明细 · 研究档案</h3>
<p>全市场207家排序 / 通道封顶预警 / 点击个股看12维明细</p>
<div class="go">点击文末「阅读原文」查看 →</div>
</div>

<div class="disclaim">
免责声明：本榜单为独立数据研究工具基于公开数据（巨潮公告、交易所行情、AkShare财务数据）自动量化生成，评分仅供研究参考，不构成任何投资建议；本平台不提供证券投资咨询服务，不推荐任何证券。ST/*ST股票退市风险高、流动性差，数据可能存在滞后或口径差异，请以交易所及上市公司正式披露为准。据此操作，风险自担。
</div>
